...
# v0.2 - 2016_07_25 -  Add choose color text (black / red or white)
# Todo
...
display_usage() {
  ...
  echo "2- couleur du texte (red / black / white) - white par defaut"
  echo " "
...
# 2 - insertion de la chaine de caractere dans l'image
#

if [ "$2" == "" ]; then
  echo "** Couleur white par defaut"
  convert /tmp/exiftmp/$avantmodif  -fill white -stroke black   -pointsize 20  -gravity north   -annotate 0 $buffer   /tmp/exiftmp/$apres

else
  echo "** Couleur $2"
  convert /tmp/exiftmp/$avantmodif  -fill $2 -stroke black   -pointsize 20  -gravity north   -annotate 0 $buffer   /tmp/exiftmp/$apres
